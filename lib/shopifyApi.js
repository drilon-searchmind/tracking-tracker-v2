 ...
 * @param {Object} config - Configuration object
 * @param {Array} facebookAdsData - Optional Facebook Ads data to merge (PS Cost)
 * @param {Array} googleAdsData - Optional Google Ads data to merge (PPC Cost)
 * @returns {Promise<Object>} Object containing metrics and totals
 */
export async function fetchShopifyDashboardMetrics(config, facebookAdsData = [], googleAdsData = []) {
    const metrics = await fetchShopifyOrderMetrics(config);

    // Merge Facebook Ads and Google Ads data with Shopify metrics
