 ...
 */
export async function fetchFacebookAdsMetrics({ accessToken, adAccountId, startDate, endDate, countryCode }) {
    ...
    // Add country filtering if countryCode is provided
    if (countryCode) {
        ...
        console.log(`Filtering Facebook Ads data by country: ${countryCode}`);
    } else {
        console.log('No country filter applied for Facebook Ads data');
    }
    ...
    try {
        ...
        for (const campaignName of topCampaignNames) {
            try {
                ...
                // Build filtering array with campaign name filter
                const filters = [{
                    field: 'campaign.name',
                    operator: 'EQUAL',
                    value: campaignName
                ...
                // Add country filter if countryCode is provided
                if (countryCode) {
                    filters.push({
                        field: 'country',
                        operator: 'IN',
