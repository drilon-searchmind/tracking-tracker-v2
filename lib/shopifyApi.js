...
/**
 * Fetch Shopify sales analytics and merge with Facebook/Google Ads data
 * Similar to fetchShopifyDashboardMetrics but uses ShopifyQL sales data instead of order metrics
 * @param {Object} config - Shopify API configuration
 * @param {Array} facebookAdsData - Current period Facebook Ads data
 * @param {Array} googleAdsData - Current period Google Ads data
 * @param {Array} lastYearFacebookAdsData - Last year Facebook Ads data
 * @param {Array} lastYearGoogleAdsData - Last year Google Ads data
 * @param {Array} twoYearsAgoFacebookAdsData - Two years ago Facebook Ads data
 * @param {Array} twoYearsAgoGoogleAdsData - Two years ago Google Ads data
 * @returns {Promise<Object>} Dashboard metrics with sales analytics
 */
export async function fetchShopifySalesAnalyticsWithAds(
    config, 
    facebookAdsData = [], 
    googleAdsData = [],
    lastYearFacebookAdsData = [],
    ...
    // Helper function to convert Shopify sales data to metrics format
    const convertSalesDataToMetrics = (salesData) => {
        return salesData.map(row => ({
            date: row.day,
            orders: parseInt(row.orders) || 0,
            revenue: parseFloat(row.total_sales) || 0, // total_sales includes everything
            revenue_ex_tax: parseFloat(row.net_sales) || 0, // net_sales is revenue minus discounts and returns
            ...
            total_tax: parseFloat(row.taxes) || 0, // Alias for consistency
            ps_cost: 0, // Will be merged from Facebook Ads
            ppc_cost: 0, // Will be merged from Google Ads
        }));
    };

    // Helper function to merge sales metrics with ads data
    const mergeWithAdsDataSales = (salesMetrics, facebookAdsData, googleAdsData) => {
        // Create maps for both ad platforms by date
        const facebookMap = {};
        facebookAdsData.forEach(row => {
            facebookMap[row.date] = row.ps_cost;
        ...
        // Merge data by date
        const mergedMetrics = Array.from(allDates).map(date => {
            const salesData = salesMetrics.find(m => m.date === date) || {
                date,
                orders: 0,
                revenue: 0,
                revenue_ex_tax: 0,
    ...
    // Current period
    const salesResult = await fetchShopifySalesAnalytics(config);
    const salesMetrics = convertSalesDataToMetrics(salesResult.salesData);
    const mergedMetrics = mergeWithAdsDataSales(salesMetrics, facebookAdsData, googleAdsData);
    const overview_metrics = calculateDerivedMetrics(mergedMetrics);
    const totals = calculateTotals(overview_metrics);
    ...
    const lastYearSalesResult = await fetchShopifySalesAnalytics(lastYearConfig);
    const lastYearSalesMetrics = convertSalesDataToMetrics(lastYearSalesResult.salesData);
    const lastYearMerged = mergeWithAdsDataSales(lastYearSalesMetrics, lastYearFacebookAdsData, lastYearGoogleAdsData);
    const last_year_metrics = calculateDerivedMetrics(lastYearMerged);
    const last_year_totals = calculateTotals(last_year_metrics);
    ...
    const twoYearsAgoSalesResult = await fetchShopifySalesAnalytics(twoYearsAgoConfig);
    const twoYearsAgoSalesMetrics = convertSalesDataToMetrics(twoYearsAgoSalesResult.salesData);
    const twoYearsAgoMerged = mergeWithAdsDataSales(twoYearsAgoSalesMetrics, twoYearsAgoFacebookAdsData, twoYearsAgoGoogleAdsData);
    const two_years_ago_metrics = calculateDerivedMetrics(twoYearsAgoMerged);
    const two_years_ago_totals = calculateTotals(two_years_ago_metrics);
