 ...
 * Also merges Facebook Ads data if provided
 * @param {Object} config - Configuration object
 * @param {Array} facebookAdsData - Optional Facebook Ads data to merge
 * @returns {Promise<Object>} Object containing metrics and totals
 */
export async function fetchShopifyDashboardMetrics(config, facebookAdsData = []) {
    const metrics = await fetchShopifyOrderMetrics(config);

    // Merge Facebook Ads data (PPC cost) with Shopify metrics
    const mergedMetrics = mergeWithFacebookAds(metrics, facebookAdsData);

    // Calculate derived metrics for each day
    const COGS = 0.7; // 70% Cost of Goods Sold
    const TAX_RATE = 0.25; // 25% Danish VAT

    const overview_metrics = mergedMetrics.map(row => ({
        ...row,
...
/**
 * Merge Facebook Ads data with Shopify metrics by date
 * @param {Array} shopifyMetrics - Shopify metrics array
 * @param {Array} facebookAdsData - Facebook Ads data array
 * @returns {Array} Merged metrics
 */
function mergeWithFacebookAds(shopifyMetrics, facebookAdsData) {
    // Create a map of Facebook Ads data by date
    const facebookMap = {};
    facebookAdsData.forEach(row => {
        facebookMap[row.date] = row.ps_cost;
    });

    // Create a map of all dates from both sources
