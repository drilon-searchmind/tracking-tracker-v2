...
const convertCurrency = (amount, fromCurrency, toCurrency = "DKK", shouldConvertCurrency = true) => {
    if (!amount || fromCurrency === toCurrency || !shouldConvertCurrency) return amount;
...
export async function queryBigQueryRollUpMetrics({ childCustomers, startDate, endDate, customerSettingsMap = {} }) {
    ...
    try {
        ...
        const customerDataSelects = childCustomers.map((customer, index) => {
            ...
            // Get customer settings to determine if currency filtering should be applied
            const customerSettings = customerSettingsMap[customer._id] || { changeCurrency: true };
            const shouldApplyCurrencyFilter = customerSettings.changeCurrency;
            ...
            console.log(`::: Currency filtering: ${shouldApplyCurrencyFilter ? 'enabled' : 'disabled'} (${customerCurrency})`);
            ...
            return `
                ...
                FROM (
                    ...
                    orders AS (
                        ...
                        ${shouldApplyCurrencyFilter ? `WHERE presentment_currency = "${customerCurrency}"` : ''}
                        ...
                        SELECT
                            ...
                            SUM(
                                ...
                                WHERE JSON_EXTRACT_SCALAR(transaction, '$.kind') = 'refund'
                                    ${shouldApplyCurrencyFilter ? `AND JSON_EXTRACT_SCALAR(transaction, '$.currency') = "${customerCurrency}"` : ''}
                                )
                            ) AS total_refunds
